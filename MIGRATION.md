# Migrating a real MinIO

For when you have an existing MinIO (or any S3 server) and a new one, and
want to copy every bucket across and prove the copy is complete. The lab's
own MinIO A and B are not used here.
...
Everything else — all settings, what each command does — is in
[GUIDE.md](GUIDE.md).
